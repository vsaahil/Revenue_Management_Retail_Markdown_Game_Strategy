...
# function to return LP optimized combo, minimizing the absolute difference between total demand and units available

def get_weeks(sales):
    prob = LpProblem("Run_Out_Rate")
    ...
    # minimize absolute deviation z (because needs to be linear)
    z = LpVariable('z', cat='Continuous')
    # total weeks need to == 14
    prob += lpSum(x_vars) == 14
    # linearization
    prob += lpSum(x_vars[i] * coeff[i] for i in range(len(variables))) - units_available <= z
    prob += -(lpSum(x_vars[i] * coeff[i] for i in range(len(variables))) - units_available) <= z
    # objective and solve
    prob.setObjective(lpSum(z))
    prob.solve()
    values = []
